...
#include<stdlib.h>
	...
	**/
	...
	{ 
		/**变量的定义**/
		int res_FH = 0, res_SX = 0, res_PLSZ = 0, res_DYDX=0;
		int m_jie = 1;//拟合阶数
		...
		double Q_unknow=0;
		FILE *fp_data=NULL;
		printf("\nThis program use a text to load the data,before start, please copy or type your data to a text,the format like \n\tZ-TAB-Q-TAB-A\n\t45\t1000\t100\n\t55\t1500\t111\nassuming your data text named 'data.txt'.\n");
		printf("*********please input your data name: ");
		scanf("%s",fileName);
		...
		{
			printf("\n\nopen your data success, starting computing...\n\n");
			/**获取行数**/
	...
	//	**/
		...
		**/
		printf("The default max order is 5, change or not? \n\t[0] not change\n\t[1]change\nplease input: ");
		scanf("%d",&m_jie);
		if(m_jie==0)
			m_jie=5;
		else
		{
			printf("input the order you want: ");
			scanf("%d",&m_jie);
		}
		//m_jie++;
		...
		//	printf("<debug--main> beta=%lf\n", beta[m]);
			...
			printf("the [measured water table Z]  [measured water flow Q] [nassuming water flow Qc] are calculated as below\n");
			printf("---N---|----Z----|----Q----|----Qc----\n");
			for(int i=0;i<n;i++)
			{
				printf("%7d|", i);
			...
			**/
			printf("\n+++++++++++++++++++++++++++++++++++++++++++++++++++\n");
			res_DYDX = chk_dydx(Q,Qc,n);
			if (res_DYDX==0)
			{
				printf("=========check DYDX fail!=========\n\n");
			}
			else
			{
				printf("=========check DYDX pass!=========\n\n");
			}
			printf("\n+++++++++++++++++++++++++++++++++++++++++++++++++++\n");
			res_FH = chk_FH(Q, Qc,n);
			if (res_FH==0)
			{
				printf("=========check FH fail!=========\n\n");
				//continue;
			}
			else
			{
				printf("=========check FH pass!=========\n\n");
			}
			printf("\n+++++++++++++++++++++++++++++++++++++++++++++++++++\n");
			res_SX = chk_SX(Q, Qc, n);
			if (res_SX==0)
			{
				printf("=========check SX fail!=========\n\n");
			}
			else
			{
				printf("=========check SX pass!=========\n\n");
			}
			printf("\n+++++++++++++++++++++++++++++++++++++++++++++++++++\n");
			...
			if (res_PLSZ==0)
			{
				printf("=========check PLSZ fail!=========\n\n");
			}
			else
			{
				printf("=========check PLSZ pass!=========\n\n");
			}
			printf("\n+++++++++++++++++++++++++++++++++++++++++++++++++++\n");
		//	printf("<debug--main>m==%d\tm_jie==%d\tsContinue==%d\t\n",m,m_jie,isContinue);
			if(m<m_jie)
			{
				...
				for(int i=0;;i++)
				{
					printf("[%d] order finished and checked \n\t[0]exit program\n\t[1]continue calculate\n\t[2]calculate Q with a known Z\nplease input: ",m);
					scanf("%d",&isContinue);
				...
				if(isContinue==1)
				{
					printf("=======continue [%d] order curve========\n\n",m+1);
				}else if(isContinue==0)
				{
					printf("[%d]order finished ,exiting...\n",m);
					break;
				}else
				{
					printf("=======calculate Qc with a known Z========\n\n");
					for(int i=0;;i++)
					{

						Q_unknow = calc_Q_unknow(Z_0, m, alpha, beta, A);
						printf("assuming flow  Q_unknow == %.2lf\ncalculate finished!\n\t[0]exit the program\n\t[1]exit and continue next order\n\t[2]calculate another Qc \ninput: ",Q_unknow);
						for(int j=0;;j++)
						{
							scanf("%d",&isContinue);
							if(isContinue==0||isContinue==1||isContinue==2)
			...
			}else
			{
				...
				for(int i=0;;i++)
				{
					printf("[%d]order finished and checked \n\t[0]exit the program\n\t[1]calculate the Qc with a known Z\ninput: ",m);
					scanf("%d",&isContinue);
					if(isContinue==0)
					{	
					...
					{
						printf("=======calculating Qc with a known water table Z========\n\n");
						for(int i=0;;i++)
						{
		
							Q_unknow = calc_Q_unknow(Z_0, m, alpha, beta, A);
							printf("assuming flow Q_unknow == %.2lf\n\ncalculate finished\n\t[0]exit \n\t[1]continue calculate Qc\ninput: ",Q_unknow);
							for(int j=0;;j++)
							{
								scanf("%d",&isContinue);
	...
	**/
	int chk_FH(double *Q, double *Q_c, int N)
	{
		...
		printf("---------default a=0.05,accept or deny?\n\t[1]accept\n\t[0]deny\ninput");
		scanf("%d", &accdny);
		if (accdny == 0 )
		{
			printf("---------input a(0.05or0.10or0.25):---------\n");
			scanf("%f", &XZXSP_tmp);
			for (int i = 0; i < 3; i++)
			{
	...
	**/
	int chk_PLSZ(double *Q,double *Qc, int N)
	{
		...
		printf("=========enter PLSZ check=========\n");
		printf("---------default a=0.05,accept or deny?\n\t[1]accept\n\t[0]deny\ninput");
		scanf("%d", &accdny);
		//printf("<debug--plsz>  accdny==%d\n", accdny);
		if (accdny == 0)
		{
				printf("---------input a(0.05 or 0.10 or 0.20 or 0.30):\n");
				scanf("%f", &XZXSP_tmp);
				for (int i = 0; i < 4; i++)
				{
					if (XZXSP_tmp==XZXSP[i])
...
double calc_Q_unknow(double Z_0, int m, double *alpha, double *beta, double *A)
{
	...
	double X[1]={0}, Z=0;
	printf("start calculate Q_unknow\n");
	printf("please input unknown water table Z= ");
	scanf("%lf",&Z);
	X[0]=log(Z - Z_0);
 ...
 *r[]为相对误差
 * */
int chk_dydx(double *Q, double *Qc, int N)
{
	int  numRiLessThan5=0;
	double *r=NULL;
	r=(double *)malloc(sizeof(double)*N);
	printf("####enter DYDX check####\n|----N----|----r----|\n");
	for(int i=0;i<N;i++)
	{
		r[i]=(Q[i]-Qc[i])/Qc[i];
		if((r[i]=r[i]*100)<0)
	...
	free(r);
	r=NULL;
	if((int)((float)numRiLessThan5/(float)N*100)<75)
		return 1;
	else
		return 0;
}
